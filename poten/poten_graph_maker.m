...
rotamethod(data.time,data.data,startT);
...
function [selected_time,selected_data]=dataselecter(time,data,startT,amount)%[a,b,c,d],[1,2,3,4],[1,3],2-->[a,b,c,d]
selected_data=[];
selected_time=[];
...
function []=graphplotter(x,y)
plot(x,y)
hold on
plot(x,y,'Marker','o')
hold on
end
function []=slidingmethod(time,data,startT)
distance=[0.2,0.7,1.2,1.7,2.2,2.7,3.2,3.7,4.2,4.7,5.2,5.7,6.3];
[t,adc]=dataselecter(time,data,startT,1000);

v=adc2percent(adc);

x=distance2percent(distance,6.3);
...
function[]=rotamethod(time,data,startT)
    degpercent=0:5:100;
    
    [t,adc]=dataselecter(time,data,startT,1000);
    v=adc2percent(adc);
    bigdeg=distance_multiplier(degpercent,1000); 
    graphplotter(bigdeg,v);
    clear
...
[t,adc]=dataselecter(time,data,startT,1000);
v=adc2V(adc);
bigdistance=distance_multiplier(distance,1000);
max=0;
min=1000;
peak=[];
for i=1:numel(v)
